...
Use the template structure from `plugins/codex-game-studio/references/templates/game-design-document.md`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   a concise plain-text question to the user for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use a concise plain-text question to the user to capture the decision.
...
**Agent delegation**: For complex mechanics, use the Codex delegation to delegate to
...
delegate to `systems-designer` via the Codex delegation. Provide:
...
**When delegating via Codex delegation**:
...
- The main session presents the agent's output to the user via a concise plain-text question to the user
...
2. **a concise plain-text question to the user** at every decision point (Explain -> Capture pattern):
