---
name: setup-engine
description: Configure the project's game engine and version. Pins the engine in docs/CODEX-STUDIO.md, detects knowledge gaps, and populates engine reference docs via WebSearch when the version is beyond the LLM's training data.
---
...
Any mention of the legacy `Task` tool or custom subagent types should be interpreted as either:
...
After updating docs/CODEX-STUDIO.md, create or update `docs/studio/technical-preferences.md` with
...
## 8. Update docs/CODEX-STUDIO.md Engine Reference

Update the "Engine Version Reference" section in `docs/CODEX-STUDIO.md` so the
active reference path points to the correct engine:
...
If the file previously pointed to a different engine, update it.

---

## 9. Optional Role Brief Refresh

If you expect to rely on Codex delegation for engine-specific work, optionally
refresh the relevant role briefs in:

`plugins/codex-game-studio/references/roles/`

When updating those briefs, make sure they direct the reader to:
...
- Always show the user what you're about to change before making docs/CODEX-STUDIO.md edits
